...
X = []
Y = []
dataset_path = 'R:\\Study\\NLP\\Project\\C50\\'
            ...
            for sent_idx in range(len(content)):
                ...
                for token_idx in range(len(tokens)):
                    ...
                    if token in glove_wordmap:
                        sentences.append(glove_wordmap[token])
                if len(sentences) != 0:
                    X.append(sentences)
...
x_train, x_test, y_train, y_test = train_test_split(X, y_onehot, test_size = 0.2, random_state = 0, shuffle=False)
